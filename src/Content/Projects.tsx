...
const Projects = () => {
  return (
    ...
    >
      ...
      >
        <p className="text-amber-500 text-xs uppercase tracking-widest mb-2" aria-hidden="true">
          What I've built
        </p>
        <h1 id="projects-heading" className="text-3xl font-bold text-white mb-8">Projects</h1>
              ...
              <article className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden hover:border-neutral-700 transition-colors">
                ...
                <div className="p-6">
                  <header className="flex items-start justify-between gap-4 mb-3">
                    ...
                    <nav aria-label={`${project.name} links`} className="flex gap-2 flex-shrink-0">
                      {project.githubUrl && (
                        <a
                  ...
                  <ul aria-label="Key highlights" className="space-y-1.5 mb-5">
                    {project.highlights.map((h) => (
                      <li key={h} className="flex gap-2 text-sm text-neutral-400">
                        <span className="text-amber-500 mt-1.5 flex-shrink-0 text-xs" aria-hidden="true">▸</span>
                        <span>{h}</span>
                      </li>
                    ))}
                  </ul>
